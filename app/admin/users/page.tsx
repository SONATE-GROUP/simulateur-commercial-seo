...
const ROLE_LABEL: Record<string, string> = { owner: 'Propriétaire', editor: 'Éditeur', reader: 'Lecteur' };
const ROLE_COLOR: Record<string, string> = { owner: '#e8571a', editor: '#4caf7d', reader: '#7a9e8e' };

interface Invitation {
  id: string; email: string; workspaceId: string | null; workspaceName: string | null;
  workspaceRole: string; expiresAt: string; acceptedAt: string | null;
  createdAt: string; status: 'pending' | 'accepted' | 'expired';
}
interface Workspace { id: string; name: string; }

const STATUS_LABEL: Record<string, string> = { pending: 'Invitation envoyée', accepted: 'Compte activé', expired: 'Invitation expirée' };
const STATUS_COLOR: Record<string, string> = { pending: '#d4a820', accepted: '#4caf7d', expired: '#888' };
...
export default function UsersPage() {
  ...
  const loadActivity = async () => {
    ...
    setActivityOpen(true); setActivityLoading(true);
  ...
  const sendInvite = async (e: React.FormEvent) => {
    ...
    } else { setInviteMsg({ ok: false, text: data.error || 'Erreur' }); setFreshLink(null); }
    setSending(false);
  };

  const resendInvite = async (inv: Invitation) => {
    setResending(inv.id);
    const res = await fetch('/api/invitations/resend', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ invitationId: inv.id }) });
    const data = await res.json();
    if (res.ok) {
      setInvitations(prev => prev.map(i => i.id === inv.id ? { ...i, status: 'pending', expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() } : i));
  ...
  const createUser = async (e: React.FormEvent) => {
    e.preventDefault(); setCreateMsg(null);
    ...
    const data = await res.json();
    if (res.ok) { setUsers(prev => [data, ...prev]); setCreateForm({ email: '', password: '', name: '' }); setCreateMsg({ ok: true, text: `Compte créé pour ${createForm.email}` }); }
    else { setCreateMsg({ ok: false, text: data.error || 'Erreur' }); }
    setCreating(false);
  };
  ...
  return (
    <div style={{ maxWidth: 1180 }}>
      ...
      <div style={{ backgroundColor: G5, borderRadius: 12, padding: 20, marginBottom: 24, border: `1px solid ${G3}` }}>
        <h2 style={{ fontSize: 13, fontWeight: 700, marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#7a9e8e' }}>Inviter un utilisateur par email</h2>
        <form onSubmit={sendInvite}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px', gap: 10, marginBottom: 10 }}>
            <input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="Email du client *" required style={{ backgroundColor: G3, border: `1px solid ${G4}`, borderRadius: 8, padding: '10px 14px', color: CREAM, fontSize: 14, outline: 'none' }} />
            <select value={inviteWorkspace} onChange={e => setInviteWorkspace(e.target.value)} style={{ backgroundColor: G3, border: `1px solid ${G4}`, borderRadius: 8, padding: '10px 14px', color: inviteWorkspace ? CREAM : '#7a9e8e', fontSize: 14, outline: 'none' }}>
              <option value="">Aucun espace</option>
              {workspaces.map(ws => <option key={ws.id} value={ws.id}>{ws.name}</option>)}
            </select>
            <select value={inviteRole} onChange={e => setInviteRole(e.target.value)} style={{ backgroundColor: G3, border: `1px solid ${G4}`, borderRadius: 8, padding: '10px 14px', color: CREAM, fontSize: 14, outline: 'none' }}>
      ...
      {/* Direct create */}
      <div style={{ backgroundColor: G5, borderRadius: 12, padding: 20, marginBottom: 24, border: `1px solid ${G3}` }}>
        <h2 style={{ fontSize: 13, fontWeight: 700, marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#7a9e8e' }}>Créer un compte directement</h2>
        <form onSubmit={createUser}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
            <input value={createForm.name} onChange={e => setCreateForm(f => ({ ...f, name: e.target.value }))} placeholder="Nom (optionnel)" style={{ backgroundColor: G3, border: `1px solid ${G4}`, borderRadius: 8, padding: '10px 14px', color: CREAM, fontSize: 14, outline: 'none' }} />
            <input type="email" value={createForm.email} onChange={e => setCreateForm(f => ({ ...f, email: e.target.value }))} placeholder="Email *" required style={{ backgroundColor: G3, border: `1px solid ${G4}`, borderRadius: 8, padding: '10px 14px', color: CREAM, fontSize: 14, outline: 'none' }} />
            <input type="password" value={createForm.password} onChange={e => setCreateForm(f => ({ ...f, password: e.target.value }))} placeholder="Mot de passe * (8 min.)" required style={{ backgroundColor: G3, border: `1px solid ${G4}`, borderRadius: 8, padding: '10px 14px', color: CREAM, fontSize: 14, outline: 'none' }} />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <button type="submit" disabled={creating} style={{ backgroundColor: G4, border: 'none', borderRadius: 8, padding: '10px 24px', color: CREAM, fontSize: 14, fontWeight: 700, cursor: creating ? 'not-allowed' : 'pointer', opacity: creating ? 0.7 : 1 }}>
              {creating ? 'Création…' : '+ Créer le compte'}
            </button>
            {createMsg && <span style={{ fontSize: 13, color: createMsg.ok ? '#4caf7d' : '#e05050' }}>{createMsg.text}</span>}
          </div>
        </form>
      </div>
      ...
      {invitations.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <h2 style={{ fontSize: 13, fontWeight: 700, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#7a9e8e' }}>Invitations</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 160px 130px 100px 100px 100px', gap: 10, padding: '4px 16px', color: '#5a7a6a', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            ...
            {invitations.map(inv => (
              <div key={inv.id} style={{ display: 'grid', gridTemplateColumns: '1fr 160px 130px 100px 100px 100px', gap: 10, alignItems: 'center', backgroundColor: G5, borderRadius: 10, padding: '12px 16px', border: `1px solid ${G3}` }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {inv.status !== 'accepted' && (
                    <>
                      <button onClick={() => resendInvite(inv)} disabled={resending === inv.id} style={{ backgroundColor: 'transparent', border: `1px solid ${G4}`, borderRadius: 6, padding: '4px 10px', color: '#7a9e8e', fontSize: 11, cursor: resending === inv.id ? 'not-allowed' : 'pointer', opacity: resending === inv.id ? 0.6 : 1, whiteSpace: 'nowrap' }}>
                        {resending === inv.id ? '…' : 'Régénérer'}
                      </button>
                      {resendLinks[inv.id] && (
                        <button onClick={() => navigator.clipboard.writeText(resendLinks[inv.id])} style={{ backgroundColor: ORANGE + '22', border: `1px solid ${ORANGE}44`, borderRadius: 6, padding: '4px 10px', color: ORANGE, fontSize: 11, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                          Copier lien
                        </button>
                      )}
                    </>
            ...
            {users.map(u => {
              ...
              <div key={u.id}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 110px 64px 90px 90px 60px 90px 90px 110px', gap: 10, alignItems: 'center', backgroundColor: isDisabled ? '#1a2e24' : G5, borderRadius: expandedUser === u.id ? '10px 10px 0 0' : 10, padding: '12px 16px', border: `1px solid ${isDisabled ? '#e0505044' : expandedUser === u.id ? G4 : G3}`, opacity: isDisabled ? 0.75 : 1 }}>
                  ...
                  <button onClick={() => toggleAccess(u.id)} title="Voir les accès" style={{ backgroundColor: (u.workspaceCount ?? 0) === 0 ? '#e0505022' : G3, color: (u.workspaceCount ?? 0) === 0 ? '#e05050' : '#7a9e8e', border: `1px solid ${(u.workspaceCount ?? 0) === 0 ? '#e0505044' : G4}`, borderRadius: 6, padding: '3px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                ...
                {expandedUser === u.id && (
                  <div style={{ backgroundColor: '#1e3529', border: `1px solid ${G4}`, borderTop: 'none', borderRadius: '0 0 10px 10px', padding: '14px 16px' }}>
                    {accessLoading === u.id ? (
                        ...
                        {(accessData[u.id] ?? []).map(ws => (
                          <div key={ws.id}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                              <Link href={`/admin/workspaces/${ws.id}`} style={{ color: CREAM, fontWeight: 600, fontSize: 13, textDecoration: 'none' }}>{ws.name}</Link>
                              <span style={{ backgroundColor: ROLE_COLOR[ws.role] + '22', color: ROLE_COLOR[ws.role], border: `1px solid ${ROLE_COLOR[ws.role]}44`, borderRadius: 5, padding: '1px 7px', fontSize: 10, fontWeight: 700 }}>{ROLE_LABEL[ws.role] ?? ws.role}</span>
                              <span style={{ color: '#5a7a6a', fontSize: 11 }}>{ws.reports.length} rapport{ws.reports.length !== 1 ? 's' : ''}</span>
                            </div>
                            {ws.reports.length > 0 && (
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 3, paddingLeft: 12, borderLeft: `2px solid ${G3}` }}>
                                {ws.reports.map(r => (
                                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                    <Link href={`/?report=${r.id}`} style={{ color: '#7a9e8e', fontSize: 12, textDecoration: 'none' }}>{r.prospect || <em>Sans nom</em>}</Link>
      ...
      <div style={{ marginTop: 32 }}>
        <button onClick={loadActivity} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', backgroundColor: G5, border: `1px solid ${G3}`, borderRadius: activityOpen ? '10px 10px 0 0' : 10, padding: '13px 18px', cursor: 'pointer', color: CREAM }}>
          <span style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#7a9e8e' }}>Activité récente</span>
          ...
          <div style={{ border: `1px solid ${G3}`, borderTop: 'none', borderRadius: '0 0 10px 10px', overflow: 'hidden' }}>
            {activityLoading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {activity.map((item, i) => (
                  <div key={item.type === 'login' ? `l-${item.userId}-${i}` : item.viewId} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 18px', backgroundColor: i % 2 === 0 ? G5 : '#1e3529', borderTop: i === 0 ? 'none' : `1px solid ${G3}` }}>
                    <span style={{ flexShrink: 0, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', borderRadius: 5, padding: '2px 7px', backgroundColor: item.type === 'login' ? '#4caf7d22' : '#3a7a9e22', color: item.type === 'login' ? '#4caf7d' : '#5ab8e8', border: `1px solid ${item.type === 'login' ? '#4caf7d44' : '#5ab8e844'}`, minWidth: 68, textAlign: 'center' }}>
                      {item.type === 'login' ? 'Connexion' : 'Rapport'}
                    </span>
                    <span style={{ fontSize: 13, fontWeight: 600, color: CREAM, minWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.userName || item.userEmail}</span>
                    ...
                    <span style={{ flexShrink: 0, fontSize: 11, color: '#5a7a6a', marginLeft: 'auto' }}>{new Date(item.date).toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', year: '2-digit', hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                ))}
              </div>
            )}
